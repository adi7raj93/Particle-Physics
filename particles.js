...
{number} nparticles initial particle count
{size} [size=5] particle size in pixels */
...
var scale = Math.floor(Math.pow(Particles.BASE, 2) / Math.max(w, h)
/ 3);
...
this.obstacles = [];
function texture() {
return igloo.texture(null, gl.RGBA, gl.CLAMP_TO_EDGE,
gl.NEAREST);
}
this.programs = {
...
Particles.BASE = 255;
/* {number} value
{number} scale to maximize use of dynamic range
...
/* Allocates textures and fills them with initial random state & returns
{Particles} */
...
/* Allocate array buffers and fill with needed values & returns {Particles} */
...
/* Set a new particle count. This is a minimum and the actual count may be
slightly higher to fill out a texture */
...
/* Swap the foreground and background state & returns {Particles} */
...
/* Brings the obstacles texture up to date & returns {Particles} */
...
/* Introduces a new circle obstacle to the simulation. We can continue to
update the obstacle position, radius, etc. so long as
we call updateObstacles() afterwards */
