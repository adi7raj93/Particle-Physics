...
{Object} [options] to pass to getContext()
{boolean} [noerror] If true, return null instead of throwing
...
{string} vertex URL or source of the vertex shader
{string} fragment URL or source of the fragment shader
{Function} [transform] Transforms the shaders before compilation
...
gl.attachShader(p, this.makeShader(gl.FRAGMENT_SHADER,
fragment));
gl.linkProgram(p);
...
/* Tell WebGL to use this program right now & returns {Igloo.Program} */
Igloo.Program.prototype.use = function() {
this.gl.useProgram(this.program);
return this;
};
/* Declare/set a uniform or set a uniform's data
{string} name uniform variable name
{number|Array|ArrayBufferView} [value]
{boolean} [i] if true use the integer version
returns {Igloo.Program} */
...
/* Like the uniform() method, but using integers & returns {Igloo.Program} */
...
{string} name attrib variable name
{WebGLBuffer} [value]
{number} [size] element size (required if value is provided)
{number} [stride=0]
returns {Igloo.Program} */
...
{number} mode
{number} count the number of vertex attribs to render
{GLenum} [type] use glDrawElements of this type
returns {Igloo.Program} */
...
/* Disables all attribs from this program & returns {Igloo.Program} */
...
{WebGLRenderingContext} gl
{GLenum} [target] either GL_ARRAY_BUFFER or
...
/* Binds this buffer to ARRAY_BUFFER & returns {Igloo.Buffer} */
...
/* {number} [unit] active texture unit to bind
returns {Igloo.Texture} */
...
{Array|ArrayBufferView|TexImageSource} source
{number} [width]
{number} [height]
returns {Igloo.Texture} */
...
var gl = this.gl;
gl.copyTexImage2D(gl.TEXTURE_2D, 0, this.format, x, y, width,
height, 0);
return this;
};
/* {WebGLRenderingContext} gl
{WebGLFramebuffer} [framebuffer] to be wrapped (null for default)
returns {Igloo.Framebuffer} */
